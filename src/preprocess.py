...
def to_df(data):
    ...
    properties= [d for d in data['features']]
    my_df = pd.json_normalize(properties)
    return my_df
...
def sort_df(my_df):
    ...
    '''
    return my_df.sort_values(by=['properties.TYPE_SITE_INTERVENTION']).reset_index(drop=True)
